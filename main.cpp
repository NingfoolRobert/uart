...
int main(int argc, char **argv)
{
    ...
    ret=pthread_create(&id2,NULL,Uart_Recv,NULL);
    if(ret)
    {
        printf("create pthread error!\n");
        return  -1;
...
int UartWrite(int fd)
{
    while(1)
    {
        pthread_mutex_lock(&p_mutex);//给互斥量上锁
        char str[SIZE]={};
        ...
        if (data_len>0)
        {
            ...
            pthread_mutex_unlock(&p_mutex);//给互斥量解锁
        }
        else
        {
            tcflush(fd, TCOFLUSH);
